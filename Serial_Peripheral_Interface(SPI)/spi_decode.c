...
int main(){
    ...
    ssp_spi_write(0x0C01); // write 1 to shutdown register for normal operation
    ...
    while(1){
      ...
      ssp_spi_write(0x010F);// write blank to digit-0 display unit
      ssp_spi_write(0x020F);// write blank to digit-1 display unit
      ssp_spi_write(0x030F);// write blank to digit-2 display unit
      ssp_spi_write(0x040F);// write blank to digit-0 display unit
...
void ssp_spi_init(void){
    LPC_GPIO0->FIODIR |= (1 << 6) ; // configure P0.6 pin as GPIO Output pin
